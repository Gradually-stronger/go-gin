...
// @Router GET /web/v1/demos?q=page
func (a *Demo) QueryPage(c *gin.Context) {
	...
	params.Status = until.S(c.Query("status")).DefaultInt(0)
...
// @Failure 401 schema.HTTPError "{error:{code:0,message:未授权}}"
// @Failure 500 schema.HTTPError "{error:{code:0,message:服务器错误}}"
// @Router PATCH /web/v1/demos/{id}/enable
...
// @Failure 401 schema.HTTPError "{error:{code:0,message:未授权}}"
// @Failure 500 schema.HTTPError "{error:{code:0,message:服务器错误}}"
// @Router PATCH /web/v1/demos/{id}/disable
