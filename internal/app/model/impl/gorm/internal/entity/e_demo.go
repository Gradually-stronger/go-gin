...
type Demo struct {
	Model
	RecordID string  `gorm:"column:record_id;size:36;index;comment:'记录内码'"` // 记录内码
	Code     *string `gorm:"column:code;size:50;index;comment:'编号'"`        // 编号
	Name     *string `gorm:"column:name;size:100;index;comment:'名称'"`       // 名称
	Memo     *string `gorm:"column:memo;size:200;comment:'备注'"`             // 备注
	Status   *int    `gorm:"column:status;index;comment:'状态(1:启用 2:停用)'"`   // 状态(1:启用 2:停用)
	Creator  *string `gorm:"column:creator;size:36;comment:'创建者'"`          // 创建者
}
